...
export class HomePage implements OnInit {
  ...
  ionViewWillEnter() {
    ...
    this.platform.ready().then(() => {
      ...
      if (this.route.snapshot.data['special']) {
        ...
        if (
          filterParams.parentType !== undefined ||
          filterParams.parentType !== -1
        ) {
          filterData['PTypeID'] = [];
        ...
        if (flag) {
          ...
          this.dataService.getFilterData(filterData).then(
            (res) => {
              if (typeof res.data === 'string') {
                //console.log('when I get the data', res.data);
  ...
  getFilter(cmd) {
    ...
    if (cmd === 'clear') {
      ...
      if (
        this.filterData.parentType !== undefined ||
        this.filterData.parentType !== -1
      ) {
      ...
      JsonObj['EventType'] = 'E';
      JsonObj['pageno'] = 0;
      this.dataService.getFilterData(JsonObj).then(
        (res) => {
          if (typeof res.data === 'string') {
            res.data = JSON.parse(res.data);
          }
  ...
  public getevents() {
    ...
    // } else {
    this.dataService.getAllEvents().then(
      (res) => {
        this.loading = false;
        if (typeof res.data === 'string') {
          res.data = JSON.parse(res.data);
